...
- `cloudflare_d1_database.main` – Cloudflare D1 serverless SQL database
...
3. **Add GitHub secrets**:
    ...
    | `TF_BUCKET_PREFIX` | Bucket prefix/path (e.g., `cloudflare-d1`) |

4. **Run the workflow**:
    - **Apply**: Go to Actions → **CD - Cloudflare D1 (Apply)** → fill in all inputs
    - **Destroy**: Go to Actions → **CD - Cloudflare D1 (Destroy)** → fill in essential inputs
